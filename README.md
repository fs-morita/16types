...
- **商品のスペック説明を書かない** — このサイトはレビューサイトではなく分析ノート。
  記事の中身は性格の分析側にあり、商品はその帰結として一言添えるだけ。
  性能を主張しないぶん、景表法(優良誤認)のリスクも小さくなる。
- **タイプ名で商品を直接紐づけない** — 4指標（E/I・N/S・T/F・J/P）に翻訳した理由を書く。
  指標ごとの指針は `mbti-types.ts` の `AXIS_GUIDANCE` にある。
- **使っていない商品の「使ってみた」を書かない** — サイト上で方針として掲げてはいないが、
  実際に使っていない商品の体験談は景表法（優良誤認）のリスクがあるため書かない。
  紹介は公開情報の整理として書く（`disclaimer` ページに明記済み）。
